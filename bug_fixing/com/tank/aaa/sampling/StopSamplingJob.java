...
public class StopSamplingJob implements org.quartz.Job {
	...
	public void execute(JobExecutionContext ctx) throws JobExecutionException {
		...
		setStartSamplingTask(ctx.getScheduler(), dpid, map, xidTmp);
	 ...
	 * @param dpid
	 */
	public void setStartSamplingTask(Scheduler scheduler, String dpid, Map<String, SwitchSamplingInfo> map, long xid) {
		...
		try {
			...
			if (xid != HashBasedUniformSampling.xid && (map.get(dpid) == null
					|| !HashBasedUniformSampling.nextTimeSws.contains(map.get(dpid).getDpid()))) {
				HashBasedUniformSampling.logger.error("sampling job: " + dpid
						+ " have no xid or xid different or sampling information. exit job! My xid: " + xid
						+ " now Xid: " + HashBasedUniformSampling.xid);
