...
public class Forwarding extends ForwardingBase implements IFloodlightModule, IOFSwitchListener {
	...
	private static boolean tcpEnable = false;
	private static boolean udpEnable = false;
	private IFlowMessageService flowMessageService = null;
	...
	public Command processPacketInMessage(IOFSwitch sw, OFPacketIn pi, IRoutingDecision decision,
		...
		if (decision != null) {
			...
			if (eth.isBroadcast() || eth.isMulticast()) {
				...
				// doForwardFlow(sw, pi, cntx, true); // @caihe
				// else {
				doForwardFlow(sw, pi, cntx, false);
				// }
			}
	...
	protected void doDropFlow(IOFSwitch sw, OFPacketIn pi, IRoutingDecision decision, FloodlightContext cntx) {
		...
		OFFlowMod.Builder fmb = sw.getOFFactory().buildFlowAdd(); // this will be a drop-flow; a flow that will not
																	// output to any ports
	...
	protected void doForwardFlow(IOFSwitch sw, OFPacketIn pi, FloodlightContext cntx,
		...
		if (dstDevice != null) {
			...
			if (on_same_if) {
				log.info("Both source and destination are on the same switch/port {}/{}. Action = NOP", sw.toString(),
						inPort);
				return;
			}
			...
			Route route = routingEngineService.getRoute(source, inPort, dstDap.getSwitchDPID(), dstDap.getPort(),
			...
			if (eth.getEtherType() == EthType.IPv4) {
				...
				if (ipv4.getProtocol() == IpProtocol.TCP && tcpEnable) {
					TCP tcp = (TCP) ipv4.getPayload();
					flow = new Flow(ipv4.getSourceAddress().getInt(), ipv4.getDestinationAddress().getInt(),
							IpProtocol.TCP.getIpProtocolNumber(), tcp.getSourcePort().getPort(),
							tcp.getDestinationPort().getPort());
				} else if (ipv4.getProtocol() == IpProtocol.UDP && udpEnable) {
					UDP udp = (UDP) ipv4.getPayload();
					flow = new Flow(ipv4.getSourceAddress().getInt(), ipv4.getDestinationAddress().getInt(),
							IpProtocol.UDP.getIpProtocolNumber(), udp.getSourcePort().getPort(),
							udp.getDestinationPort().getPort());
				}
			}
			if(flow != null)requestFlowRemovedNotifn = true;
			...
			if (route != null) {
				...
				pushRoute(route, m, pi, sw.getId(), cookie, cntx, requestFlowRemovedNotifn, OFFlowModCommand.ADD);
				if (flow != null) {
					FlowAddMessage flowMsg = new FlowAddMessage.Builder(flow, route).build();
					flowMessageService.pubMessage(flowMsg);
					//log.info("Publish a flow add message."+flow+"; "+route);
				}

				// log.info(route.toString());
			} else {
				/* Route traverses no links --> src/dst devices on same switch */
				log.debug("Could not compute route. Devices should be on same switch src={} and dst={}", srcDevice,
						dstDevice);
				Route r = new Route(srcDevice.getAttachmentPoints()[0].getSwitchDPID(),
				...
				r.setPath(path);
				pushRoute(r, m, pi, sw.getId(), cookie, cntx, requestFlowRemovedNotifn, OFFlowModCommand.ADD);
				if (flow != null) {
					FlowAddMessage flowMsg = new FlowAddMessage.Builder(flow, r).build();
					flowMessageService.pubMessage(flowMsg);
					//log.info("Publish a flow add message."+flow+"; "+r);
				}
			}
		} else {
	...
	protected Match createMatchFromPacket(IOFSwitch sw, OFPort inPort, FloodlightContext cntx) {
		...
		if (eth.getEtherType() == EthType.IPv4) { /* shallow check for equality is okay for EthType */
			...
			if (FLOWMOD_DEFAULT_MATCH_IP_ADDR) {
				mb.setExact(MatchField.ETH_TYPE, EthType.IPv6).setExact(MatchField.IPV6_SRC, srcIp)
	...
	public void init(FloodlightModuleContext context) throws FloodlightModuleException {
		...
		this.flowMessageService = context.getServiceImpl(IFlowMessageService.class);
		...
		tcpEnable = Boolean.parseBoolean(configParameters.get("tcp"));
		udpEnable = Boolean.parseBoolean(configParameters.get("udp"));
		log.info("TCP capture: " + tcpEnable + "; UDP capture: " + udpEnable);
	}
